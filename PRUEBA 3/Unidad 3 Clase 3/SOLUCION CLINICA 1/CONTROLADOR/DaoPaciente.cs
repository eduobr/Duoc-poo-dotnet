...
namespace CONTROLADOR
{
    public class DaoPaciente:IDaoPaciente
    {
        ...
        public int MayoresIsapre() {
            int mayores_isapre = pacientes.Where(p => p.Edad >= 18 && p.Salud == TipoSalud.Isapre).Count();
            return mayores_isapre;
        }
        public int AdultosMenores13() {
            int am13 = pacientes.Where(p => p.Hora < 13 && p.GetType() == typeof(CL_ADULTO)).Count();
            return am13;
        }
        public double PromedioEdadNiños() {
            double pro = pacientes.Where(p => p.GetType() == typeof(CL_NINO)).Average(p => p.Edad);
            return pro;
        }
    }
}
